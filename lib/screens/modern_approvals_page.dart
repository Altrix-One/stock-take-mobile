...
import 'package:cohenix_ess/hr/services/approvals_service.dart';
...
class _ModernApprovalsPageState extends State<ModernApprovalsPage>
  ...
  Widget _buildQuickStats() {
    ...
    final averageTime =
        _approvalsStats['average_approval_time']?.toString() ?? '0';
  ...
  Widget _buildPendingApprovals() {
    ...
    return Column(
      ...
      children: [
        ...
        ...(_pendingApprovals.map((approval) => _buildApprovalItem(
  ...
  Widget _buildApprovalItem(Map<String, dynamic> approval,
    ...
    if (fromDate != null && toDate != null) {
      ...
      subtitle +=
          ' • Submitted: ${submittedDate.split(' ')[0]}'; // Just the date part
    }
    ...
    return ModernInfoCard(
      title: title.isNotEmpty
          ? title
          : _generateBetterTitle(approval, type, requesterName),
      subtitle: subtitle,
  ...
  Widget _buildMonthlyOverview() {
    ...
    final thisMonthApproved =
        _approvalsStats['this_month_approved']?.toString() ?? '0';
    final thisMonthRejected =
        _approvalsStats['this_month_rejected']?.toString() ?? '0';
    final myPendingCount =
        _approvalsStats['my_pending_count']?.toString() ?? '0';
    ...
    return ModernHeroCard(
      ...
      child: Row(
        children: [
          Expanded(
            child: _buildOverviewItem('Approved', thisMonthApproved,
                Icons.check_circle, ModernDesignSystem.success),
          ),
          Expanded(
            child: _buildOverviewItem('Pending', myPendingCount,
                Icons.pending_actions, ModernDesignSystem.warning),
          ),
          Expanded(
            child: _buildOverviewItem('Rejected', thisMonthRejected,
                Icons.cancel, ModernDesignSystem.error),
          ),
        ],
      ),
    );
  ...
  Widget _buildApprovalHistory() {
    ...
    return Column(
      ...
      children: [
        ...
        ...(_myApprovals.take(10).map((approval) =>
            _buildApprovalItem(approval as Map<String, dynamic>))),
  ...
  Widget _buildTeamOverview() {
    final activeMembers =
        _teamMembers.where((member) => member['is_active'] == true).length;
    final onLeaveToday = _approvalsStats['on_leave_today']?.toString() ?? '0';
    final avgTeamRating =
        _approvalsStats['avg_team_rating']?.toString() ?? '0.0';
    ...
    return ModernHeroCard(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: _buildOverviewItem('On Leave Today', onLeaveToday,
                Icons.event_busy, ModernDesignSystem.warning),
          ),
          Expanded(
            child: _buildOverviewItem('Team Rating', avgTeamRating, Icons.star,
  ...
  Widget _buildTeamMembers() {
    ...
    return Column(
      ...
      children: [
        ...
        ...(_teamMembers.map(
            (member) => _buildTeamMemberItem(member as Map<String, dynamic>))),
      ],
    );
  }
  ...
  Widget _buildTeamMemberItem(Map<String, dynamic> member) {
    ...
    final name = member['employee_name']?.toString() ??
        ...
        'Unknown Employee';
    final role =
        member['designation']?.toString() ?? member['role']?.toString() ?? '';
  ...
  Future<void> _rejectRequest(Map<String, dynamic> approval) async {
    ...
    try {
      // Determine doctype based on approval type or use a generic approach
      final doctype = _getDocTypeFromApproval(approval);
      await ApprovalsService.rejectRequest(doctype, approval['id'].toString(),
          reason: reason);
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
  ...
  String _generateBetterTitle(
      Map<String, dynamic> approval, String type, String requesterName) {
    final leaveType = approval['leave_type']?.toString();
  ...
  String _getDocTypeFromApproval(Map<String, dynamic> approval) {
    ...
    switch (type) {
      ...
      default:
        ...
        if (approval['from_date'] != null && approval['reason'] != null)
          return 'Attendance Request';
...
class ApprovalDetailsBottomSheet extends StatelessWidget {
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                children: [
                  // Basic Information
                  _buildDetailItem(
                      context, 'Request Type', _getDisplayType(approval)),
                  _buildDetailItem(
                      context, 'Title/Subject', _getDisplayTitle(approval)),
                  _buildDetailItem(
                      context, 'Requested By', _getRequesterName(approval)),
                  _buildDetailItem(
                      context, 'Status', _getDisplayStatus(approval)),

                  // Date Information
                  if (_getSubmittedDate(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Submitted Date', _getSubmittedDate(approval)),
                  if (_getFromDate(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'From Date', _getFromDate(approval)),
                  ...
                  if (_getLeaveType(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Leave Type', _getLeaveType(approval)),
                  if (_getTotalDays(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Total Days', _getTotalDays(approval)),
                  ...
                  if (_getTotalClaimedAmount(approval).isNotEmpty)
                    _buildDetailItem(context, 'Claimed Amount',
                        _getTotalClaimedAmount(approval)),

                  // Description/Reason
                  if (_getDescription(approval).isNotEmpty)
                    _buildDetailItem(context, 'Description/Reason',
                        _getDescription(approval)),
                  ...
                  if (_getDepartment(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Department', _getDepartment(approval)),

                  // Approval workflow information
                  if (_getApprover(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Approver', _getApprover(approval)),
                  if (_getApprovalDate(approval).isNotEmpty)
                    _buildDetailItem(
                        context, 'Approval Date', _getApprovalDate(approval)),
                  if (_getRejectionReason(approval).isNotEmpty)
                    _buildDetailItem(context, 'Rejection Reason',
                        _getRejectionReason(approval)),

                  // Attachments
                  if (approval['attachments'] != null &&
                      (approval['attachments'] as List).isNotEmpty)
                    _buildAttachmentsSection(
                        context, approval['attachments'] as List),
  ...
  Widget _buildDebugSection(
      BuildContext context, Map<String, dynamic> approval) {
    return Container(
          ...
          if (approval.isNotEmpty) ...[
            ...
            ...approval.entries.take(10).map((entry) => Padding(
                  ...
                  child: Text(
                    ...
                    style: ModernDesignSystem.captionLarge.copyWith(
                      color: ModernDesignSystem.getTextTertiary(
                          Theme.of(context).brightness),
                    ),
                  ),
                )),
              ...
              child: Column(
                children: [
                  _buildDetailItem(
                      context,
                      'Full Name',
                      member['employee_name']?.toString() ??
                          member['name']?.toString() ??
                          'Unknown'),
                  _buildDetailItem(
                      context, 'Employee ID', member['name']?.toString() ?? ''),
                  _buildDetailItem(
                      context,
                      'Designation',
                      member['designation']?.toString() ??
                          ...
                          ''),
                  _buildDetailItem(context, 'Department',
                      member['department']?.toString() ?? ''),
                  _buildDetailItem(context, 'Status',
                      (member['is_active'] ?? true) ? 'Active' : 'Inactive'),
                  if (member['user_id'] != null)
                    _buildDetailItem(
                        context, 'Email', member['user_id']?.toString() ?? ''),
                  if (member['date_of_joining'] != null)
                    _buildDetailItem(context, 'Date of Joining',
                        member['date_of_joining']?.toString() ?? ''),
                  if (member['last_seen'] != null)
                    _buildDetailItem(context, 'Last Seen',
                        member['last_seen']?.toString() ?? 'Never'),
                  _buildDetailItem(context, 'Pending Requests',
                      member['pending_requests']?.toString() ?? '0'),
                ],
              ),
            ),
          ),
...
class ApprovalsHistoryPage extends StatelessWidget {
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return _buildHistoryItem(
                        approvals[index] as Map<String, dynamic>);
                  },
                ),
              ),
            ],
  ...
  Widget _buildHistoryItem(Map<String, dynamic> approval) {
    ...
    // Add amount or days
    final amount = approval['amount']?.toString() ??
        approval['total_claimed_amount']?.toString();
    final totalDays = approval['total_leave_days']?.toString();
    ...
    if (approvedDate.isNotEmpty) {
      final actionText =
          status.toLowerCase() == 'approved' ? 'Approved' : 'Processed';
