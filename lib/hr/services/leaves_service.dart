...
class LeavesService {
  ...
  static Future<List<dynamic>> leaveTypes() async {
    try {
      ...
      final res =
          await HrmsApiClient.postMethod('hrms.api.get_leave_types', params: {
        'employee': emp,
  ...
  static Future<Map<String, dynamic>> getLeaveApprovalDetails(
      String employee) async {
    final res = await HrmsApiClient.postMethod(
        'hrms.api.get_leave_approval_details',
        params: {
          'employee': employee,
        });
    return res['message'] as Map<String, dynamic>? ?? {};
  ...
  static Future<double> getNumberOfLeaveDays(
      {required String fromDate,
      required String toDate,
      required String leaveType,
      String? employee,
      bool halfDay = false,
      String? halfDayDate}) async {
    final res = await HrmsApiClient.postMethod(
  ...
  static Future<double> getLeaveBalanceOn(
  ...
  static Future<void> cancelLeaveApplication(String name,
      {String? reason}) async {
    // Try dedicated HRMS endpoint first
    try {
      await HrmsApiClient.postMethod('hrms.api.cancel_leave_application',
          params: {
            'name': name,
  ...
  static Future<String?> resolveLeaveName(
      {required String leaveType,
      required String fromDate,
      required String toDate,
      String? employee}) async {
    try {
      ...
      final r = await HrmsApiClient.getJson('/api/resource/Leave%20Application',
          query: {
            'fields': '["name","status"]',
            'filters':
                '[ ["Leave Application","leave_type","=","$leaveType"], ["Leave Application","from_date","=","$fromDate"], ["Leave Application","to_date","=","$toDate"]${emp != null ? ',["Leave Application","employee","=","$emp"]' : ''} ]',
  ...
  static Future<Map<String, dynamic>> leaveBalanceWithPending() async {
    ...
    try {
      ...
      pending.forEach((lt, days) {
        ...
        final used = (map['leaves_taken'] is num)
            ? (map['leaves_taken'] as num).toDouble()
            : double.tryParse('${map['leaves_taken']}') ?? 0;
        final bal = (map['balance_leaves'] is num)
            ? (map['balance_leaves'] as num).toDouble()
            : double.tryParse('${map['balance_leaves']}') ?? 0;
