...
class ErrorMessageParser {
  ...
  static String parseLeaveApplicationError(String errorMessage) {
    if (errorMessage.isEmpty) return 'An unknown error occurred. Please try again.';
    
    // Convert to lowercase for easier matching
    final lowerError = errorMessage.toLowerCase();
    
    // Check for overlap errors
    if (lowerError.contains('overlapError') || lowerError.contains('overlap') || 
    ...
    if (lowerError.contains('date') && (lowerError.contains('invalid') || 
        lowerError.contains('past') || lowerError.contains('future'))) {
    ...
    if (lowerError.contains('401') || lowerError.contains('authentication') ||
        lowerError.contains('token') || lowerError.contains('login')) {
    ...
    try {
      if (errorMessage.contains('{')) {
        ...
        if (errorData['exception'] != null) {
          ...
          return parseLeaveApplicationError(exception); // Recursive call with extracted message
        }
  ...
  static String parseGeneralError(String errorMessage, {String operation = 'operation'}) {
    if (errorMessage.isEmpty) return 'An unknown error occurred during $operation.';
  ...
  static String getErrorIcon(String errorMessage) {
    ...
    if (lowerError.contains('overlap') || lowerError.contains('already applied')) {
