...
def execute():
	...
	if frappe_v.startswith("14") and hrms_v.startswith("15"):
		message = f"""
			The `develop` branch of Cohenix HR is no longer compatible with Frappe & ERPNext's `version-14`.
			Since you are using ERPNext/Frappe `version-14` please switch Cohenix HR's branch to `version-14` and then proceed with the update.\n\t
